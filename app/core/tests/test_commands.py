...
# postgres를 django에서 dockero compose와 함께 사용할 때, postgres가 준비가 되지 않은 상태에서 django 앱이 데이터베이스에 연결하려고 하면 에러가 발생한다. 
# 따라서 프로젝트의 안정성을 향상시키기 위해서 docker compose에서 실행할 모든 명령 전에 이 도우미 명령을 추가하고, 데이터베이스에 엑세스 하기 전에 준비가 되었는지를 확인한다. 

# 먼저, patch func. 이것을 통해 우리는 django get database function의 행동(behavior)을 mock할 수 있다. 
...
class CommandTests(TestCase):
    ...
    def test_wait_for_db_ready(self):
        ...
        # 따라서 테스트를 설정하기 위해 ConnectionHandler의 동작을 오버라이드할 것이다. true 반환할 거고, exception을 반환하지 않을 것이다. 
